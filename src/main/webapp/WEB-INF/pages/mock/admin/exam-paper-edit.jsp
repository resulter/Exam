    ...
    <div class="main">
        ...
        <div class="row">
            <div class="form-group col-md-offset-1 col-xs-2">
                ...
                <input type="text" class="form-control" id="ExamTitle" name="name" value="${localExam.name}">
            </div>
        </div>
        <div class="row">
            <div class="col-md-2 col-md-offset-1">
                ...
                <select class="form-control" id="readingSelect" name="readingSectionId">
                    <c:forEach items="${dataList.readingSelect}" var="sel">
                        <c:choose>
                            <c:when test="${sel.key==localExam.readingSectionId}">
                                <option value="${sel.key }" selected="selected" class="opt">${sel.value }</option>
                            </c:when>
                            <c:otherwise>
                                <option value="${sel.key }" class="opt">${sel.value }</option>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
                </select>
            ...
            <div class="col-md-2">
                ...
                <select class="form-control" id="listeningSelect" name="listeningSectionId">
                    <c:forEach items="${dataList.listeningSelect}" var="sel">
                        <c:choose>
                            <c:when test="${sel.key==localExam.listeningSectionId}">
                                <option value="${sel.key }" selected="selected" class="opt">${sel.value }</option>
                            </c:when>
                            <c:otherwise>
                                <option value="${sel.key }" class="opt">${sel.value }</option>
            ...
            <div class="col-md-2">
                ...
                <select class="form-control" id="speakingSelect" name="speakingSectionId">
                    <c:forEach items="${dataList.speakingSelect}" var="sel">
                        <c:choose>
                            <c:when test="${sel.key==localExam.speakingSectionId}">
                                <option value="${sel.key }" selected="selected" class="opt">${sel.value }</option>
                            </c:when>
                            <c:otherwise>
                                <option value="${sel.key }" class="opt">${sel.value }</option>
            ...
            <div class="col-md-2">
                ...
                <select class="form-control" id="writingSelect" name="writingSectionId">
                    <c:forEach items="${dataList.writingSelect}" var="sel">
                        <c:choose>
                            <c:when test="${sel.key==localExam.writingSectionId}">
                                <option value="${sel.key }" selected="selected" class="opt">${sel.value }</option>
                            </c:when>
                            <c:otherwise>
                                <option value="${sel.key }" class="opt">${sel.value }</option>
        ...
        <div class="row" style="margin-top: 5px">
            <div class="form-group col-md-offset-1 col-xs-2">
                ...
                <input type="text" class="form-control" id="ExamRemark" name="reamrk" value="${localExam.reamrk}">
            </div>
        </div>
    </div>
</form>
<div class="row" style="padding-top: 15px">
    ...
    <div class="btn ue-clear">
        <a href="javascript:;" class="confirm" onclick="editExamPaper(${localExam.id})">修改</a>
    </div>
    </div>
</div>
</body>
...
<script type="text/javascript">
    ...
    //更新
    function editExamPaper(id) {
        document.myform.attributes["action"].value = "${ctx}/mock/editExamPaper.action?paperId=" + id;
        $("form").submit();
    }
